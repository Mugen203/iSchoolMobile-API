...
public class DashboardService
{
    ...
    private async Task<(decimal OutstandingBalance, DateTimeOffset? NextDueDate)> GetFinancialStatusAsync(
        string studentID)
    {
        // Get the total amount owed
        var pendingFees = await _context.FeeItems
            .Include(f =>
                f.FinancialRecord) // Eager load FinancialRecord to access Student (though StudentID is not directly used in this query now)
            .Where(f => f.FinancialRecord.StudentID == studentID &&
                        f.PaymentStatus == Enums.PaymentStatus.Pending) // Now PaymentStatus is directly on FeeItem
            .ToListAsync();
    ...
    private async Task<NextClassInfo?> GetNextClassInfoAsync(string studentID)
    {
        ...
        //TODO: Refactor this to display proper defaults
        ...
        foreach (var course in courseStudents.Select(courseStudent => courseStudent.Course))
        {
            ...
            if (lecturer != null) lecturerName = $"{lecturer.LecturerFirstName} {lecturer.LecturerLastName}";

            // Get the first timeslot (in the real implementation, find the next timeslot based on current time)
